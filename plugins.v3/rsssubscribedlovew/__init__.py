...
class RssSubscribedlovew(_PluginBase):
            ...
            for result in results:
                try:
                    ...
                    history.append({
                        ...
                        "media_source": mediainfo.media_source.value if getattr(mediainfo, "media_source", None) else None,
                        "media_id": str(mediainfo.media_id) if getattr(mediainfo, "media_id", None) else None,
    ...
    def __add_strong_subscribe(self, subscribechain, meta, mediainfo, rss_url: str, platform: str):
        ...
        if not (media_source and media_id) and getattr(mediainfo, "tmdb_id", None):
            # 回退到 TMDB 单源身份，保证订阅仍带完整主身份
