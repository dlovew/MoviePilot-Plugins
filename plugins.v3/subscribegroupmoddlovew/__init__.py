...
from app.schemas.types import EventType, SystemConfigKey, MediaType, MediaSource
from app.sdk.events import Event, eventmanager
from app.sdk.logging import logger
from app.sdk.media import MetaVideo, Tokens
...
class SubscribeGroupModdlovew(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "电视剧下载后自动添加官组等信息到订阅；添加订阅后根据二级分类名称自定义订阅规则。订阅时支持从标题识别流媒体平台。"
    # 插件图标
    ...
    def subscribe_notice(self, event: Event = None):
        ...
        # 流媒体平台可独立于二级分类填充生效
        platform_enabled = "流媒体平台" in (self._update_details or [])

        if not self._category and not platform_enabled:
            logger.error("二级分类自定义填充未开启")
            return

        if len(self._subscribe_confs.keys()) == 0 and not platform_enabled:
        ...
        event_media = event_data.get("mediainfo")
        category = event_media.get("category")

        # V3 已移除 tmdbid，改用统一媒体身份 media_source + media_id 识别二级分类
        if not category:
            media_source = event_media.get("media_source")
            media_id = event_media.get("media_id")
            media_type = event_media.get("type")
            if not media_source or not media_id or not media_type:
                logger.error(f"订阅ID:{sid} 媒体身份不完整，无法识别二级分类")
                ...
                logger.info(f"订阅ID:{sid} 未获取到二级分类，尝试通过媒体信息识别")
        ...
        if self._category and category and category not in self._subscribe_confs.keys():
        ...
        if not subscribe:
            logger.warning(f"订阅ID:{sid} 已不存在，跳过规则更新")
            return

        update_dict = {}

        # 二级分类自定义配置
        if self._category and category:
        ...
        # 流媒体平台填充（dlovew 魔改：订阅时即从标题识别）
        if platform_enabled:
            title = event_media.get("title") or subscribe.name
            web_source = self._get_streaming_platform_from_title(title) if title else ""
            if web_source:
                current_include = update_dict.get('include') or subscribe.include or ""
                update_dict['include'] = f"{current_include}(?=.*{web_source})"
        ...
        history.append({
            ...
            # ensure_ascii=True 避免中文在存储层二次编码后出现乱码
    ...
    def download_notice(self, event: Event = None):
        ...
        if not media_source or not media_id:
            logger.warning(f"下载历史:{download_history.title} 媒体身份不完整，跳过处理")
            return

        # 已处理记录：兼容 V2 时代仅 type:tmdbid 的旧 key
        history_key = f"{download_history.type}:{media_source}:{media_id}"
        legacy_tmdb_key = f"{download_history.type}:{media_id}"
        handled = history_key in history_handle
        if str(media_source).lower() in {MediaSource.TMDB.value, "tmdb"}:
            handled = handled or legacy_tmdb_key in history_handle
        if handled:
        ...
        if not subscribes:
            logger.warning(
                f"下载历史:{download_history.title} 媒体身份:{media_source}:{media_id} 对应订阅记录不存在")
            return

        logger.info(
            f"获取到媒体身份 {media_source}:{media_id} season {season} 订阅记录:{len(subscribes)} 个")
        ...
        for subscribe in subscribes:
            ...
            if not context:
                logger.warning("下载事件上下文为空，跳过订阅规则填充")
                return
            ...
            # 保存已处理历史
            if history_key not in history_handle:
                history_handle.append(history_key)
