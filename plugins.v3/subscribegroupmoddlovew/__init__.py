...
class SubscribeGroupModdlovew(_PluginBase):
    ...
    def download_notice(self, event: Event = None):
        ...
        media_source, media_id = self.__get_media_identity(download_history)
        identity_key = (
            f"{download_history.type}:{media_source.value}:{media_id}"
            if media_source and media_id else f"{download_history.type}:"
        )
        if identity_key in history_handle:
            return
        ...
        # V3 订阅查询已改为统一媒体身份；list_by_media_identity 不支持季过滤，需自行筛选
        subscribes = self._subscribeoper.list_by_media_identity(
            media_source=media_source,
            media_id=media_id,
        ) or []
        if season is not None:
            subscribes = [item for item in subscribes if getattr(item, "season", None) == season]
        if not subscribes:
            return
        ...
        for subscribe in subscribes:
            ...
            if update_dict:
                ...
                self.save_data(key="history", value=history)
                history_handle.append(identity_key)
    ...
    def __get_media_identity(item) -> tuple:
        ...
        if legacy_tmdbid and str(legacy_tmdbid).strip():
            return MediaSource.TMDB, str(legacy_tmdbid).strip()
        return None, None
