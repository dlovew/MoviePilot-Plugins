...
class QbTorrentFixerdlovew(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._last_log = []
        # 已处理历史从插件数据恢复，避免重启/重载后「数据」页丢失
        self._processed = self.get_data('processed') or []
    ...
    def scan_and_fix(self, manual: bool = False):
        ...
        with self._lock:
            ...
            # 与历史合并去重后持久化，避免插件重启/重载后「数据」页丢失
            if mixed_torrents:
                history: List[Dict[str, Any]] = self.get_data('processed') or []
                index_map = {(h.get('name'), h.get('downloader')): i for i, h in enumerate(history)}
                for item in mixed_torrents:
                    key = (item.get('name'), item.get('downloader'))
                    if key in index_map:
                        history[index_map[key]] = item
                    else:
                        history.append(item)
                # 按时间倒序，仅保留最近 200 条
                history = sorted(history, key=lambda x: x.get('time') or '', reverse=True)[:200]
                self._processed = history
                self.save_data('processed', history)
            else:
                self._processed = self.get_data('processed') or []
            logger.info(summary)
