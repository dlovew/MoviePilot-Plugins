...
class DownloadSiteTagModdlovew(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            saved_rename = config.get("all_cat_rename")
            # V3 分类体系已迁移为版本化策略，分类数量与名称可能变化；
            # 保存的重命名列表长度不匹配时重置为当前分类列表，避免按下标取值越界
            if not saved_rename or len(saved_rename) != len(self._all_cat):
                self._all_cat_rename = self._all_cat
            else:
                self._all_cat_rename = saved_rename
            ...
            if not self._rename_type:
                for index, item in enumerate(self._all_cat):
                    self._cat_rename_dict[str(item)] = (
                        self._all_cat_rename[index]
                        if index < len(self._all_cat_rename)
                        else str(item)
                    )
    ...
    @staticmethod
    def _is_random_temp_tag(tag: str) -> bool:
        """
        判断是否为 qBittorrent 模块添加下载时生成的 10 位随机临时标签(见 MoviePilot issue #6171)。
        需排除用户配置的 TORRENT_TAG(如默认 MOVIEPILOT 也是 10 位字母数字串, 不能误删)。
        """
        if not tag:
            return False
        if settings.TORRENT_TAG and tag == settings.TORRENT_TAG:
            return False
        return bool(re.fullmatch(r'[A-Za-z0-9]{10}', tag))
            ...
            for torrent in torrents:
                try:
                    ...
                    # 如果站点名称为空, 尝试通过trackers识别
                        ...
                        if not history.torrent_site and not self.__get_media_id(history) and not history.type and not history.title:
                    ...
                    if service.type == "qbittorrent" and self._enable_category and not torrent_cat and history.type and not self._rename_type:
                        ...
                        media_source, media_id = self.__get_media_identity(history)
                        if media_source and media_id and history_type:
                            tmdb_info = self.__get_tmdb_info(media_source, media_id, history_type)
                            if tmdb_info:
                                # 确定二级分类
                    ...
                    if _cat == torrent_cat:
                        ...
                        _cat = None
                    # 判断当前种子是否不需要修改(含需要清理的随机临时标签时仍需执行)
                    _has_random = any(self._is_random_temp_tag(t) for t in torrent_tags)
                    if not _cat and not _tags and not _has_random:
    ...
    def _set_torrent_info(self, service: ServiceInfo, _hash: str, _torrent: Any = None, _tags=None, _cat: str = None,
        ...
        if _hash and _torrent:
            ...
            if service.type == "qbittorrent":
                # 清理 qBittorrent 模块添加下载时生成的 10 位随机临时标签残留(见 MoviePilot issue #6171)
                try:
                    _current_tags = self._get_label(_torrent, service.type)
                except Exception:
                    _current_tags = []
                _random_tags = [t for t in _current_tags if self._is_random_temp_tag(t)]
                if _random_tags and getattr(downloader_obj, "qbc", None):
                    try:
                        downloader_obj.qbc.torrents_removeTags(hashes=_hash, tags=_random_tags)
                    except Exception as e:
                        logger.error(f"{self.LOG_TAG}清理随机临时标签失败: {str(e)}")
                # 设置标签
                if _tags:
    ...
    def download_added(self, event: Event):
        ...
        try:
            ...
            if self._enable_category and _media.type and not self._rename_type:
                # tmdb_id获取tmdb信息
                tmdb_info = self.__get_tmdb_info(
                    getattr(_media, "media_source", None),
                    getattr(_media, "media_id", None),
                    _media.type,
                    fallback_tmdbid=getattr(_media, "tmdb_id", None),
                )
                if tmdb_info:
                    # 确定二级分类
                    if tmdb_info.get('media_type') == MediaType.TV:
    ...
    @staticmethod
    def __get_media_identity(item) -> tuple:
        """
        读取统一媒体身份 (media_source, media_id)。

        V3 的下载历史已移除 tmdbid 字段，改为 media_source + media_id；
        这里同时兼容极旧数据上残留的 tmdbid。
        """
        media_source = getattr(item, "media_source", None)
        media_id = getattr(item, "media_id", None)
        if media_source and media_id is not None and str(media_id).strip():
            return str(media_source), str(media_id).strip()
        legacy_tmdbid = getattr(item, "tmdbid", None)
        if legacy_tmdbid and str(legacy_tmdbid).strip():
            return MediaSource.TMDB.value, str(legacy_tmdbid).strip()
        return None, None

    def __get_media_id(self, item):
        """返回可用于判空的媒体 ID，无有效身份时返回 None。"""
        _, media_id = self.__get_media_identity(item)
        return media_id

    def __get_tmdb_info(self, media_source, media_id, mtype, fallback_tmdbid=None):
        """
        按统一媒体身份获取 TMDB 详情，用于计算二级分类。

        TMDB 来源直接使用 media_id；其它来源先识别得到媒体信息再取 TMDB 详情。
        """
        tmdbid = None
        if media_source and media_id:
            if str(media_source) == MediaSource.TMDB.value:
                tmdbid = media_id
            else:
                try:
                    ...
                    logger.debug(f"按统一身份识别媒体失败：{err}")
                    mediainfo = None
                if mediainfo:
                    tmdbid = getattr(mediainfo, "tmdb_id", None)
        if not tmdbid and fallback_tmdbid:
            tmdbid = fallback_tmdbid
        if not tmdbid:
            return None
        try:
            return self.chain.tmdb_info(mtype=mtype, tmdbid=tmdbid)
        except Exception as err:  # noqa: BLE001
            logger.debug(f"获取 TMDB 信息失败：{err}")
            return None
